...
####Alternative - RoundRobin and UnifiedJedis
There is another, very simple but effective strategy for distribution: "RoundRobin" simply cycles through the available slaves. To use it, simply use "[RoundRobinPool](https://gist.github.com/1084272)". Whereas JedisPool serves several instances of deeply-equal objects to allow multithreading, RoundRobinPool serves heterogenous objects, Jedis instances of the different slaves in the network. Just as with JedisPool, call getRessource() on it to get a Jedis instance and don't forget returnResource after usage. The advantage is that no hashing occurs, and distribution is independent on the key name, hence more equal. In future versions, it allows redundancy, and multi-slave download of i.e. bigger lists/sortedsets (not yet implemented). The downside is that each slave has to manage the whole dataset, not 1/n of it.
Since it can be cumbersome to call getResource and returnResource all the time (only then you actually cycle through the slaves and distribute the load!), there is a utility class called [UnifiedJedis](https://gist.github.com/1159990) which abstracts away the pool operations and also greatly simplifies the general usage: it implements all Jedis commands and delegates writes to the master, and reads to cycling Jedis instances. Besides ease of use, it also avoids difficult to track errors: if by negligence you direct a write process to a slave, there is no error, the slave accepts and saves it just fine, but the change is not replicated and eventually overwritten. This can no longer happen. Furthermore, since it calls get/returnResource for each single Jedis method of both master and RoundRobin, exhaustion of the pool by forgetting returnResource is avoided, and load is distributed very equally.
